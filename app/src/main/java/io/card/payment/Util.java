...
class Util {
	...
	private static final boolean TORCH_BLACK_LISTED = (Build.MODEL
			.equals("DROID2"));
	...
	public static String manifestHasConfigChange(ResolveInfo resolveInfo,
			Class activityClass) {
		String error = null;
		if (resolveInfo == null) {
	...
	private static boolean hardwareSupportCheck() {
		...
		if (c == null) {
			...
			List<Camera.Size> list = c.getParameters()
	...
	static public Rect rectGivenCenter(Point center, int width, int height) {
		return new Rect(center.x - width / 2, center.y - height / 2, center.x
				+ width / 2, center.y + height / 2);
	}
