...
/**
 * This class implements a transparent overlay that is drawn over the raw camera
 * capture frames.
 * <p/>
 * OverlayView draws the guide frame which indicates to the user where to hold
 * the card. For debug builds, it also displays the frame rate and the focus
 * score. Once a card is detected, the class displays a still image of the card.
 * <p/>
 * There are two stages of mark-up that are applied to the card image. When the
 * image is first passed into this class, a negative rounded rectangle mask is
 * used to block out the image background behind the rounded corners of the
 * card. Then, a light gray rounded rectangle outline is drawn along the card
 * edges.
 * <p/>
 * Once the digits are detected for the credit card number, those digits are
 * drawn above the respective digits of the card.
 * <p/>
 * An instance of this class is created when the owning CardIOActivity is
 * created. Its lifecycle is the same as that of the owning activity.
 * <p/>
 * <p/>
 * A couple of technical notes:
 * <p/>
 * the drawing code is not optimized for performance. It re-computes values for
 * each frame that could be cached instead (such as guide tick locations).
 * However, I have measured performance for the app, including drawing
 * performance, and drawing is not at all a bottleneck. So, fixing this seems
 * like a low priority item.
 * <p/>
 * It is not clear whether the rotation animation currently implemented is
 * really needed. We should figure out a strategy for this, and then change the
 * code accordingly.
 * <p/>
 * To prevent race conditions & memory leaks, setting new card images happens in
 * the context of the UI thread.
 * <p/>
 * The class has an instance float field displayScale that holds the value
 * screen dimensions are scaled by. This field is used for instance to achieve
 * consistent guide frame thickness independent of screen scale.
 * <p/>
 */
class OverlayView extends View {
	...
	private static final float GUIDE_LINE_HEIGHT = GUIDE_FONT_SIZE
			+ GUIDE_LINE_PADDING;
	...
	private static final Orientation[] GRADIENT_ORIENTATIONS = {
			Orientation.TOP_BOTTOM, Orientation.LEFT_RIGHT,
			Orientation.BOTTOM_TOP, Orientation.RIGHT_LEFT };
	...
	// Keep paint objects around for high frequency methods to avoid
	// re-allocating them.
	...
	public OverlayView(CardIOActivity captureActivity,
			AttributeSet attributeSet, boolean showTorch) {
	...
	public void setGuideAndRotation(Rect rect, int rotation) {
		...
		if (mRotation % 180 != 0) {
			topEdgeUIOffset = new Point((int) (40 * mScale),
					(int) (60 * mScale));
			mRotationFlip = -1;
		} else {
			topEdgeUIOffset = new Point((int) (60 * mScale),
					(int) (40 * mScale));
			mRotationFlip = 1;
		}
		if (mCameraPreviewRect != null) {
			Log.d(TAG, "" + mCameraPreviewRect + ", " + topEdgeUIOffset + ", "
					+ mCameraPreviewRect + ", " + topEdgeUIOffset);
			Point torchPoint = new Point(mCameraPreviewRect.left
					+ topEdgeUIOffset.x, mCameraPreviewRect.top
					+ topEdgeUIOffset.y);

			// mTorchRect used only for touch lookup, not layout
			mTorchRect = Util
					.rectGivenCenter(torchPoint, (int) (TORCH_WIDTH * mScale),
							(int) (TORCH_HEIGHT * mScale));

			// mLogoRect used only for touch lookup, not layout
			Point logoPoint = new Point(mCameraPreviewRect.right
					- topEdgeUIOffset.x, mCameraPreviewRect.top
					+ topEdgeUIOffset.y);
			mLogoRect = Util.rectGivenCenter(logoPoint,
					(int) (LOGO_MAX_WIDTH * mScale),
			...
			mGradientDrawable = new GradientDrawable(gradientOrientation,
					gradientColors);
			...
			mLockedBackgroundPath.addRect(new RectF(mCameraPreviewRect),
					Path.Direction.CW);
			mLockedBackgroundPath
	...
	public void onDraw(Canvas canvas) {
		...
		if ((mRotation == 0) || (mRotation == 180)) {
			tickLength = (mGuide.bottom - mGuide.top) / 12;
		} else {
			tickLength = (mGuide.right - mGuide.left) / 12;
		}
		...
		// top left
		canvas.drawRect(
				guideStrokeRect(mGuide.left, mGuide.top, mGuide.left
						+ tickLength, mGuide.top), mGuidePaint);
		canvas.drawRect(
				guideStrokeRect(mGuide.left, mGuide.top, mGuide.left,
						mGuide.top + tickLength), mGuidePaint);

		// top right
		canvas.drawRect(
				guideStrokeRect(mGuide.right, mGuide.top, mGuide.right
						- tickLength, mGuide.top), mGuidePaint);
		canvas.drawRect(
				guideStrokeRect(mGuide.right, mGuide.top, mGuide.right,
						mGuide.top + tickLength), mGuidePaint);
		...
		// bottom left
		canvas.drawRect(
				guideStrokeRect(mGuide.left, mGuide.bottom, mGuide.left
						+ tickLength, mGuide.bottom), mGuidePaint);
		canvas.drawRect(
				guideStrokeRect(mGuide.left, mGuide.bottom, mGuide.left,
						mGuide.bottom - tickLength), mGuidePaint);

		// bottom right
		canvas.drawRect(
				guideStrokeRect(mGuide.right, mGuide.bottom, mGuide.right
						- tickLength, mGuide.bottom), mGuidePaint);
		canvas.drawRect(
				guideStrokeRect(mGuide.right, mGuide.bottom, mGuide.right,
						mGuide.bottom - tickLength), mGuidePaint);
		...
		if (mDInfo != null) {
			if (mDInfo.topEdge) {
				canvas.drawRect(
						guideStrokeRect(mGuide.left, mGuide.top, mGuide.right,
								mGuide.top), mGuidePaint);
			}
			if (mDInfo.bottomEdge) {
				canvas.drawRect(
						guideStrokeRect(mGuide.left, mGuide.bottom,
								mGuide.right, mGuide.bottom), mGuidePaint);
			}
			if (mDInfo.leftEdge) {
				canvas.drawRect(
						guideStrokeRect(mGuide.left, mGuide.top, mGuide.left,
			...
			// float y = -(((guideHeight * (lines.length - 1)) - guideFontSize)
			// / 2) - 3;
			//
		...
		if (!hideCardIOLogo) {
			...
			canvas.rotate(mRotationFlip * mRotation);
			mLogo.draw(canvas, LOGO_MAX_WIDTH * mScale, LOGO_MAX_HEIGHT
					* mScale);
			canvas.restore();
		}
		...
		// // draw torch
		// canvas.save();
		// canvas.translate(mTorchRect.exactCenterX(),
		// mTorchRect.exactCenterY());
		// canvas.rotate(mRotationFlip * mRotation);
		// mTorch.draw(canvas);
		// canvas.restore();
		// }
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		// Rect r = Util.rectGivenCenter(p, BUTTON_TOUCH_TOLERANCE,
		// BUTTON_TOUCH_TOLERANCE);
		// Log.d(TAG, "onTouchEvent: " + p);
		// if (mShowTorch && mTorchRect != null
		// && Rect.intersects(mTorchRect, r)) {
		...
		// // Un-reproducible NPE reported on device without flash where flash
		// // detected and flash
		// // button pressed (see
		// // https://github.com/paypal/PayPal-Android-SDK/issues/27)
	...
	private void decorateBitmap() {
		...
		RectF roundedRect = new RectF(2, 2, mBitmap.getWidth() - 2,
				mBitmap.getHeight() - 2);
		float cornerRadius = mBitmap.getHeight() * CORNER_RADIUS_SIZE;

		// Alpha canvas with white rounded rect
		Bitmap maskBitmap = Bitmap.createBitmap(mBitmap.getWidth(),
		...
		maskCanvas.drawRoundRect(roundedRect, cornerRadius, cornerRadius,
				maskPaint);
	...
	// TODO - move this into RequestTask, so we just get back a card image ready
	// to go
	public void markupCard() {
		...
		for (int i = 0; i < len; i++) {
			...
			bc.drawText("" + mDetectedCard.cardNumber.charAt(i), xOffset,
					yOffset, paint);
		}
	}
