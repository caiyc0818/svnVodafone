...
class Logo {
	...
	void loadLogo(boolean useCardIOLogo) {
		...
		if (useCardIOLogo) {
			mLogo = ViewUtil.base64ToBitmap(Base64EncodedImages.card_io_logo,
					mContext, density);
		} else {
			mLogo = ViewUtil.base64ToBitmap(Base64EncodedImages.paypal_logo,
					mContext, density);
		}
	...
	public void draw(Canvas canvas, float maxWidth, float maxHeight) {
		...
		canvas.drawBitmap(mLogo,
				new Rect(0, 0, mLogo.getWidth(), mLogo.getHeight()), new RectF(
