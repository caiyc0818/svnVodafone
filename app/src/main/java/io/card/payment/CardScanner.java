 ...
 * <p/>
 * 1. CardIOActivity sets up the CardScanner, Preview and Overlay. 2. As each
 * frame is received & processed by the scanner, the scanner notifies the
 * activity of any relevant changes. (e.g. edges detected, scan complete etc.)
 * 3. CardIOActivity passes on the information to the preview and overlay, which
 * can then update themselves as needed. 4. Once a result is reported,
 * CardIOActivty closes the scanner and launches the next activity.
 * <p/>
 * HOWEVER, at the moment, the CardScanner is directly communicating with the
 * Preview.
 */
class CardScanner implements Camera.PreviewCallback, Camera.AutoFocusCallback,
	...
	private native void nGetGuideFrame(int orientation, int previewWidth,
			int previewHeight, Rect r);

	private native void nScanFrame(byte[] data, int frameWidth,
			int frameHeight, int orientation, DetectionInfo dinfo,
			Bitmap resultBitmap, boolean scanExpiry);
	...
	int mPreviewWidth = 640;// MyApplication.screenHeight;
	int mPreviewHeight = 480;// MyApplication.screenWidth;
	...
	static {
		// Log.i(Util.PUBLIC_LOG_TAG, "card.io " + BuildConfig.PRODUCT_VERSION +
		// " " + BuildConfig.BUILD_TIME);

		try {
			System.loadLibrary("cardioDecider");
			Log.d(Util.PUBLIC_LOG_TAG,
					"Loaded card.io decider library.  nUseNeon():" + nUseNeon()
							+ ",nUseTegra():" + nUseTegra());
	...
	CardScanner(CardIOActivity scanActivity, int currentFrameOrientation) {
		...
		if (scanIntent != null) {
			mSuppressScan = scanIntent.getBooleanExtra(
					CardIOActivity.EXTRA_SUPPRESS_SCAN, false);
			mScanExpiry = scanIntent.getBooleanExtra(
					CardIOActivity.EXTRA_REQUIRE_EXPIRY, false)
					&& scanIntent.getBooleanExtra(
							CardIOActivity.EXTRA_SCAN_EXPIRY, true);
		}
	...
	/**
	 * Connect or reconnect to camera. If fails, sleeps and tries again. Returns
	 * <code>true</code> if successful, <code>false</code> if maxTimeout passes.
	 */
	private Camera connectToCamera(int checkInterval, int maxTimeout) {
		...
		if (useCamera) {
			do {
				try {
					// Camera.open() will open the back-facing camera. Front
					// cameras are not
					// attempted.
					return Camera.open();
					...
					try {
						...
						Log.e(Util.PUBLIC_LOG_TAG,
								"Interrupted while waiting for camera", e1);
					}
				} catch (Exception e) {
					Log.e(Util.PUBLIC_LOG_TAG,
							"Unexpected exception. Please report it to support@card.io",
							e);
					maxTimeout = 0;
				}
	...
	void prepareScanner() {
		...
		if (useCamera && mCamera == null) {
			mCamera = connectToCamera(CAMERA_CONNECT_RETRY_INTERVAL,
					CAMERA_CONNECT_TIMEOUT);
			if (mCamera == null) {
				Log.e(Util.PUBLIC_LOG_TAG,
						"prepare scanner couldn't connect to camera!");
				return;
			} else {
			...
			if (supportedPreviewSizes != null) {
				for (Size s : supportedPreviewSizes) {
					if (s.width == MyApplication.getLarge()
							|| s.height == MyApplication.getLess()) {
						previewSize = s;
						break;
					}
				}
				if (previewSize == null) {
					...
					previewSize.width = MyApplication.getLarge();
					previewSize.height = MyApplication.getLess();
				}
			}
	...
	public void pauseScanning() {
		...
		Log.i(TAG, "scan paused"); // tests look for this message. don't delete
									// it.
	}
	...
	private boolean makePreviewGo(SurfaceHolder holder) {
		...
		if (useCamera) {
			...
			try {
				...
				Log.e(Util.PUBLIC_LOG_TAG,
						"startPreview failed on camera. Error: ", e);
				return false;
			}
		}
		return true;
	...
	public void onPreviewFrame(byte[] data, Camera camera) {
		...
		/** pika **/
		nScanFrame(data, mPreviewWidth, mPreviewHeight, mFrameOrientation,
				dInfo, detectedBitmap, mScanExpiry);
	...
	Map<String, Object> getAnalytics() {
		...
		analytics.put("num_frames_scanned",
				Integer.valueOf(nGetNumFramesScanned()));
		analytics.put("num_frames_skipped", Integer.valueOf(numFramesSkipped));

		analytics.put("elapsed_time", Double.valueOf((System
				.currentTimeMillis() - captureStart) / 1000));

		analytics.put("num_manual_refocusings",
				Integer.valueOf(numManualRefocus));
		analytics.put("num_auto_triggered_refocusings",
				Integer.valueOf(numAutoRefocus));
		analytics.put("num_manual_torch_changes",
				Integer.valueOf(numManualTorchChange));
		return analytics;
	}
	...
	public boolean setFlashOn(boolean b) {
		if (mCamera != null) {
			...
			try {
				Camera.Parameters params = mCamera.getParameters();
				params.setFlashMode(b ? Parameters.FLASH_MODE_TORCH
