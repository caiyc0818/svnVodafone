...
/**
 * This is the entry point {@link android.app.Activity} for a card.io client to
 * use <a href="https://card.io">card.io</a>.
 *
 * @version 1.0
 */
public final class CardIOActivity extends Activity {
	/**
	 * Boolean extra. Optional. Defaults to <code>false</code>. If set, the card
	 * will not be scanned with the camera.
	 */
	public static final String EXTRA_NO_CAMERA = "io.card.payment.noCamera";

	/**
	 * Boolean extra. Optional. Defaults to <code>false</code>. If set to
	 * <code>false</code>, expiry information will not be required.
	 */
	public static final String EXTRA_REQUIRE_EXPIRY = "io.card.payment.requireExpiry";

	/**
	 * Boolean extra. Optional. Defaults to <code>true</code>. If set to
	 * <code>true</code>, and {@link #EXTRA_REQUIRE_EXPIRY} is <code>true</code>
	 * , an attempt to extract the expiry from the card image will be made.
	 */
	public static final String EXTRA_SCAN_EXPIRY = "io.card.payment.scanExpiry";

	/**
	 * Boolean extra. Optional. Defaults to <code>false</code>. If set, the user
	 * will be prompted for the card CVV.
	 */
	public static final String EXTRA_REQUIRE_CVV = "io.card.payment.requireCVV";

	/**
	 * Boolean extra. Optional. Defaults to <code>false</code>. If set, the user
	 * will be prompted for the card billing postal code.
	 */
	public static final String EXTRA_REQUIRE_POSTAL_CODE = "io.card.payment.requirePostalCode";

	/**
	 * Boolean extra. Optional. Defaults to <code>false</code>. If set, the
	 * card.io logo will be shown instead of the PayPal logo.
	 */
	public static final String EXTRA_USE_CARDIO_LOGO = "io.card.payment.useCardIOLogo";

	/**
	 * Parcelable extra containing {@link CreditCard}. The data intent returned
	 * to your {@link android.app.Activity}'s
	 * {@link Activity#onActivityResult(int, int, Intent)} will contain this
	 * extra if the resultCode is {@link #RESULT_CARD_INFO}.
	 */
	...
	/**
	 * Boolean extra. Optional. Defaults to <code>false</code>. Removes the
	 * keyboard button from the scan screen.
	 * <p/>
	 * If scanning is unavailable, the {@link android.app.Activity} result will
	 * be {@link #RESULT_SCAN_NOT_AVAILABLE}.
	 */
	public static final String EXTRA_SUPPRESS_MANUAL_ENTRY = "io.card.payment.suppressManual";

	/**
	 * String extra. Optional. The preferred language for all strings appearing
	 * in the user interface. If not set, or if set to null, defaults to the
	 * device's current language setting. <br/>
	 * <br/>
	 * Can be specified as a language code ("en", "fr", "zh-Hans", etc.) or as a
	 * locale ("en_AU", "fr_FR", "zh-Hant_TW", etc.). <br/>
	 * <br/>
	 * If the library does not contain localized strings for a specified locale,
	 * then will fall back to the language. E.g., "es_CO" -> "es". <br/>
	 * If the library does not contain localized strings for a specified
	 * language, then will fall back to American English. <br/>
	 * <br/>
	 ...
	 * device's current region as well. E.g., specifying "en" on a device set to
	 * "English" and "United Kingdom" will result in "en_GB". <br/>
	 * <br/>
	 * These localizations are currently included: <br/>
	 * <p/>
	 * da, de, en, en_AU, en_GB, es, es_MX, fr, he, is, it, ja, ko, nb, nl, pl,
	 * pt, pt_BR, ru, sv, tr, zh-Hans, zh-Hant, zh-Hant_TW.
	 */
	...
	/**
	 * Boolean extra. Optional. Defaults to <code>false</code>. When set to
	 * <code>true</code> the card.io logo will not be shown overlaid on the
	 * camera.
	 */
	public static final String EXTRA_HIDE_CARDIO_LOGO = "io.card.payment.hideLogo";

	/**
	 * String extra. Optional. Used to display instructions to the user while
	 * they are scanning their card.
	 */
	public static final String EXTRA_SCAN_INSTRUCTIONS = "io.card.payment.scanInstructions";

	/**
	 * Boolean extra. Optional. Once a card image has been captured but before
	 * it has been processed, this value will determine whether to continue
	 * processing as usual. If the value is <code>true</code> the
	 * {@link CardIOActivity} will finish with a {@link #RESULT_SCAN_SUPPRESSED}
	 * result code.
	 */
	public static final String EXTRA_SUPPRESS_SCAN = "io.card.payment.suppressScan";

	/**
	 * String extra. If {@link #EXTRA_RETURN_CARD_IMAGE} is set to
	 * <code>true</code>, the data intent passed to your
	 * {@link android.app.Activity} will have the card image stored as a JPEG
	 * formatted byte array in this extra.
	 */
	public static final String EXTRA_CAPTURED_CARD_IMAGE = "io.card.payment.capturedCardImage";

	/**
	 * Boolean extra. Optional. If this value is set to <code>true</code> the
	 * card image will be passed as an extra in the data intent that is returned
	 * to your {@link android.app.Activity} using the
	 * {@link #EXTRA_CAPTURED_CARD_IMAGE} key.
	 */
	public static final String EXTRA_RETURN_CARD_IMAGE = "io.card.payment.returnCardImage";

	/**
	 * Integer extra. Optional. If this value is provided the view will be
	 * inflated and will overlay the camera during the scan process. The integer
	 * value must be the id of a valid layout resource.
	 */
	...
	/**
	 * Boolean extra. Optional. If this value is set to <code>true</code>, and
	 * the application has a theme, the theme for the card.io
	 * {@link android.app.Activity}s will be set to the theme of the
	 * application.
	 */
	 ...
	 * {@link Activity#onActivityResult(int, int, Intent)} when a scan request
	 * completes.
	 */
	...
	/**
	 * result code supplied to
	 * {@link Activity#onActivityResult(int, int, Intent)} when the user presses
	 * the cancel button.
	 */
	 ...
	 * {@link #EXTRA_SUPPRESS_MANUAL_ENTRY} is set and scanning is not
	 * available.
	 * <p/>
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (numActivityAllocations != 1) {
			// it seems that this can happen in the autotest loop, but it
			// doesn't seem to break.
			// was happening for lemon... (ugh, long story) but we're now
		...
		useApplicationTheme = clientData.getBooleanExtra(
				CardIOActivity.EXTRA_KEEP_APPLICATION_THEME, false);
		...
		errorMsg = Util.manifestHasConfigChange(resolveInfo,
				CardIOActivity.class);
		if (errorMsg != null) {
			throw new RuntimeException(errorMsg); // Throw the actual exception
													// from this class, for
			// clarity.
		}

		suppressManualEntry = clientData.getBooleanExtra(
				EXTRA_SUPPRESS_MANUAL_ENTRY, false);

		if (clientData.getBooleanExtra(EXTRA_NO_CAMERA, false)) {
			Log.i(Util.PUBLIC_LOG_TAG,
					"EXTRA_NO_CAMERA set to true. Skipping camera.");
			manualEntryFallbackOrForced = true;
		} else {
			try {
				if (!Util.hardwareSupported()) {
					StringKey errorKey = StringKey.ERROR_NO_DEVICE_SUPPORT;
					String localizedError = LocalizedStrings
							.getString(errorKey);
				...
				Toast toast = Toast.makeText(this, localizedError,
						Toast.LENGTH_LONG);
		...
		if (!manualEntryFallbackOrForced) {
			...
			try {
				...
				if (clientData.getBooleanExtra(
						PRIVATE_EXTRA_CAMERA_BYPASS_TEST_MODE, false)) {
					if (!this.getPackageName().contentEquals(
							"io.card.development")) {
						Log.e(TAG, this.getPackageName() + " is not correct");
						throw new IllegalStateException(
								"illegal access of private extra");
					}
					// use reflection here so that the tester can be safely
					// stripped for release
					// builds.
					Class<?> testScannerClass = Class
							.forName("io.card.payment.CardScannerTester");
					Constructor<?> cons = testScannerClass.getConstructor(
							this.getClass(), Integer.TYPE);
					mCardScanner = (CardScanner) cons.newInstance(new Object[] {
							this, mFrameOrientation });
				} else {
		...
		if (manualEntryFallbackOrForced) {
			if (suppressManualEntry) {
				Log.i(Util.PUBLIC_LOG_TAG,
						"Camera not available and manual entry suppressed.");
	...
	private void handleGeneralExceptionError(Exception e) {
		...
		Log.e(Util.PUBLIC_LOG_TAG,
				"Unkown exception - please send the stack trace to support@card.io",
				e);
	...
	private void doOrientationChange(int orientation) {
		...
		if (orientation < DEGREE_DELTA || orientation > 360 - DEGREE_DELTA) {
			degrees = 0;
			mFrameOrientation = ORIENTATION_PORTRAIT;
		} else if (orientation > 90 - DEGREE_DELTA
				&& orientation < 90 + DEGREE_DELTA) {
			degrees = 90;
			mFrameOrientation = ORIENTATION_LANDSCAPE_LEFT;
		} else if (orientation > 180 - DEGREE_DELTA
				&& orientation < 180 + DEGREE_DELTA) {
			degrees = 180;
			mFrameOrientation = ORIENTATION_PORTRAIT_UPSIDE_DOWN;
		} else if (orientation > 270 - DEGREE_DELTA
				&& orientation < 270 + DEGREE_DELTA) {
			degrees = 270;
			mFrameOrientation = ORIENTATION_LANDSCAPE_RIGHT;
		}
		if (degrees >= 0 && degrees != mLastDegrees) {
			Log.d(TAG, "onOrientationChanged(" + degrees
					+ ") calling setDeviceOrientation(" + mFrameOrientation
					+ ")");
	...
	protected void onPause() {
		super.onPause();
		Log.i(TAG,
				"onPause() xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_CARD_INFO
				|| resultCode == RESULT_ENTRY_CANCELED
				|| manualEntryFallbackOrForced) {
			if (data != null && data.hasExtra(EXTRA_SCAN_RESULT)) {
				Log.v(TAG,
						"data entry result: "
								+ data.getParcelableExtra(EXTRA_SCAN_RESULT));
			}
	 ...
	 * <p/>
	 * An ARM7 processor and Android SDK 8 or later are required. Additional
	 * checks for specific misbehaving devices may also be added.
	 *
	 * @return <code>true</code> if camera is supported. <code>false</code>
	 *         otherwise.
	 */
	...
	public static Bitmap getCapturedCardImage(Intent intent) {
		...
		Bitmap result = BitmapFactory.decodeStream(inStream, null,
	...
	void onCardDetected(Bitmap detectedBitmap, DetectionInfo dInfo) {
		...
		try {
			...
			Log.w(Util.PUBLIC_LOG_TAG,
					"Exception while attempting to vibrate: ", e);
		}
		...
		Bitmap scaledCard = Bitmap
				.createBitmap(detectedBitmap, 0, 0, detectedBitmap.getWidth(),
		...
		if (mDetectOnly) {
			...
			scaledCard
					.compress(Bitmap.CompressFormat.JPEG, 80, scaledCardBytes);

			Intent dataIntent = new Intent();
			if (getIntent() != null
					&& getIntent().getBooleanExtra(EXTRA_RETURN_CARD_IMAGE,
							false)) {
				dataIntent.putExtra(EXTRA_CAPTURED_CARD_IMAGE,
	...
	private void nextActivity() {
		...
		if (origIntent != null
				&& origIntent.getBooleanExtra(EXTRA_SUPPRESS_CONFIRMATION,
						false)) {
			Intent dataIntent = new Intent(CardIOActivity.this,
			...
			if (origIntent.getBooleanExtra(EXTRA_RETURN_CARD_IMAGE, false)
				...
				ByteArrayOutputStream scaledCardBytes = new ByteArrayOutputStream();
				mOverlay.getBitmap().compress(Bitmap.CompressFormat.JPEG, 80,
						scaledCardBytes);
				dataIntent.putExtra(EXTRA_CAPTURED_CARD_IMAGE,
						scaledCardBytes.toByteArray());
			}
			...
			new Handler().post(new Runnable() {
				...
				public void run() {
					...
					Intent intent = new Intent(CardIOActivity.this,
							DataEntryActivity.class);

					if (mOverlay != null) {
						mOverlay.markupCard();
						if (markedCardImage != null
								&& !markedCardImage.isRecycled()) {
					...
					if (mDetectedCard != null) {
						...
						/*
						 * add extra to indicate manual entry. This can
						 * obviously be indicated by the presence of
						 * EXTRA_SCAN_RESULT. The purpose of this is to ensure
						 * there's always an extra in the DataEntryActivity. If
						 * there are no extras received by DataEntryActivity,
						 * then an error has occurred.
						 */
						intent.putExtra(EXTRA_MANUAL_ENTRY_RESULT, true);
					}

					intent.putExtras(getIntent()); // passing on any received
													// params (such as isCvv
					// and language)
	...
	private void showErrorMessage(final String msgStr) {
		...
		Toast toast = Toast.makeText(CardIOActivity.this, msgStr,
				Toast.LENGTH_LONG);
		toast.show();
	}
	...
	private boolean restartPreview() {
		...
		boolean success = mCardScanner.resumeScanning(mPreview
				.getSurfaceHolder());
		if (success) {
			mUIBar.setVisibility(View.VISIBLE);
		}
	...
	private void setDeviceDegrees(int degrees) {
		...
		if (MyApplication.getLarge() > 1750) {
			mGuideFrame.top += sv.getTop();
			mGuideFrame.bottom += sv.getTop();// 治标不治本
		} else {
			mGuideFrame.top += sv.getTop() + 65;
			mGuideFrame.bottom += sv.getTop() - 65;// 治标不治本
		}
	...
	void setFlashOn(boolean b) {
		boolean success = (mPreview != null && mOverlay != null && mCardScanner
				.setFlashOn(b));
		if (success) {
	...
	private void setPreviewLayout() {
		...
		if (MyApplication.getLarge() > 1750) {
			mPreview = new Preview(this, null, mCardScanner.mPreviewWidth,
					mCardScanner.mPreviewHeight);
		} else {
			mPreview = new Preview(this, null, MyApplication.getLarge(),
					MyApplication.getLess());
		}
		...
		mOverlay.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT,
				LayoutParams.MATCH_PARENT));
		if (getIntent() != null) {
			boolean useCardIOLogo = getIntent().getBooleanExtra(
					EXTRA_USE_CARDIO_LOGO, false);
			...
			if (color != 0) {
				...
				if (color != alphaRemovedColor) {
					Log.w(Util.PUBLIC_LOG_TAG,
							"Removing transparency from provided guide color.");
				}
				mOverlay.setGuideColor(alphaRemovedColor);
			} else {
				// default to greeeeeen
				mOverlay.setGuideColor(Color.RED);
			}

			boolean hideCardIOLogo = getIntent().getBooleanExtra(
					EXTRA_HIDE_CARDIO_LOGO, false);
			mOverlay.setHideCardIOLogo(hideCardIOLogo);

			String scanInstructions = getIntent().getStringExtra(
		...
		if (!suppressManualEntry) {
			...
			keyboardBtn.setMinimumHeight(ViewUtil
					.typedDimensionValueToPixelsInt(
			...
			ViewUtil.setPadding(keyboardBtn,
					Appearance.CONTAINER_MARGIN_HORIZONTAL, null,
					Appearance.CONTAINER_MARGIN_HORIZONTAL, null);
			ViewUtil.setMargins(keyboardBtn, Appearance.BASE_SPACING,
					Appearance.BASE_SPACING, Appearance.BASE_SPACING,
					Appearance.BASE_SPACING);
		...
		if (getIntent() != null) {
			...
			int resourceId = getIntent().getIntExtra(
					EXTRA_SCAN_OVERLAY_LAYOUT_ID, -1);
			if (resourceId != -1) {
				customOverlayLayout = new LinearLayout(this);
				customOverlayLayout.setLayoutParams(new LayoutParams(
						LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
				...
				LinearLayout layLayout = (LinearLayout) customOverlayLayout
						.findViewById(R.id.second_title);
				layLayout.setBackgroundResource(R.color.half_dack_transparency);
				layLayout.setPadding(0, 0, 0, 10);
				((TextView) customOverlayLayout.findViewById(R.id.title))
						.setText("Top Up");
				((ImageView) customOverlayLayout.findViewById(R.id.back))
						.setVisibility(View.GONE);
				customOverlayLayout.findViewById(R.id.close)
