...
/**
 * This class contains a SurfaceView, which is used to display the camera
 * preview frames. It performs basic layout and life cycle tasks for the camera
 * and camera previews.
 * <p/>
 * Technical note: display of camera preview frames will only work when there is
 * a valid surface view to display those frames on. To that end, I have added a
 * valid surface flag that is updated by surface view lifecycle callbacks. We
 * only attempt (re-)start camera preview if there is a valid surface view to
 * draw on.
 */
class Preview extends ViewGroup {
	...
	public Preview(Context context, AttributeSet attributeSet,
		...
		// which is part of cardScanner. see
	...
	// TODO - document
	// Need a better explanation of why onMeasure is needed and how width/height
	// are determined.
	// Must the camera be set first via setCamera? What if
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Log.d(TAG,
				String.format(
						"Preview.onMeasure(w:%d, h:%d) setMeasuredDimension(w:%d, h:%d)",
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && getChildCount() > 0) {
			...
			// Log.i(TAG,
			// String.format("onLayout() - child measurements: {w:%d, h:%d}",
			// getMeasuredWidth(), getMeasuredHeight()));

			// Center the child SurfaceView within the parent, making sure that
			// the preview is
